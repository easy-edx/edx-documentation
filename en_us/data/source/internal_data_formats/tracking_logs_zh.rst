...
本节提供数据包传递中的事件数据的参考信息。事件由服务器、浏览器或移动设备发射，它们捕捉到LMS中课程页面和讲师控制板的交互信息，并将其存储至JSON文档中。数据包中的事件数据由日志文件提供。
...
本节将学生和课程小组事件分为同一类别。
对于事件的完整列表, 请参阅 :ref:`event_list`，事件按字母顺序排序。
...
该案例用于删除被编辑过的个人身份信息，而事件本身存储在JSON文件中，在标准格式化前难以阅读。如下是来至edX.log文件中的一个示例：
...
如果你使用一个JSON格式化程序更美观打印出这个事件，那么一种更可读的版本边诞生了。
...
关于字段的更多信息包含在每一个事件中，请参阅
:ref:`common`. 有关 ``problem_check``事件和其他类型事件的详细信息， 请参阅 :ref:`Student_Event_Types` 或
...
本节介绍适用于所有事件的模式定义的JSON字段。这些字段信息存放在事件的JSON文件的根目录下。

本节按字母表顺序列出常见字段，你数据包中的真实事件则以不同的序列包含着这些字段。
...
**详细信息：** 该值来自HTTP Accept-Language request-header 字段. 欲了解更多信息, 请参阅HTTP/1.1 header中 
`Accept-Language`_ 字段的定义。
...
``context`` 字段包含能提供上下文信息的成员字段。

* 此字段包含了一组适用于所有事件的核心成员字段。
* 对于有额外上下文需求的特定事件，此字段包含了一组额外的仅适用于这些事件的成员字段。
* 对于任何情况，本字段还可以包括一个或多个附加成员字段。有关一个事件成员字段的详细信息，请参阅本节后面关于此事件的描述。

``context`` 成员字段公共的所有事件
***********************************************

在所有事件的 ``context`` 字段中，均会出现以下成员字段。
...
.. 注意:: 有时候，一个事件会被用缺失或空白的
 ``context.user_id`` 值所记录. 当用户注销，或登录会话超时而浏览器窗口保持打开状态时会发生这种情况。后续操作会被记录，但是系统不能提供用户标识符。Edx建议您忽略分析过程中的这些事件。

针对适用事件的的``context`` 成员字段
******************************************************

当 ``context`` 字段适用于一个事件时，它还包括这些提供额外信息的成员字段。
...
.. list-table::
   ...
   * - ``course_user_tags``
     - object
     - 包含 ``user_api_usercoursetag``
       列表的键（s）和值（s） 请参阅 :ref:`user_api_usercoursetag`.
   * - ``module``
     - object
     - 为参与服务器事件的组件提供识别信息。

       例如，在一个服务器事件 ``problem_check`` 中,  ``module`` 字段指示服务器成功检查出来的问题组件。
       成员字段是 ``display_name`` 和 ``usage_key``。

       模块如果被用于呈现来自库中内容的课程，``module`` 将包括 ``original_usage_key`` 和
       ``original_usage_version`` 字段。这些成员字段提供一个一致的方式以确定组件来自同一个库源，并且可用于识别源库。 


如果不能确定值，那么 ``context`` 成员字段为空。

**历史**: ``usage_key`` 2015年1月28日增加； ``path`` 2014年5月7日增加；
``course_user_tags`` 2014年3月12日增加； ``user_id`` 2014年11月6日增加； 其他事件字段可重复这份数据：2013年10月23如增加。
...
**详细信息：** 该字段包括识别每一个触发事件的具体成员字段。不同的成员字段为不同的事件提供。有关``event``成员字段的更多信息，请参阅本节后面对事件的描述。
...
**历史**：2014年10月16日更新，可识别从移动设备发出的事件。
...
**详细信息：** 被触发事件的类型的值取决于 ``event_source``。本节后半部分中

:ref:`Student_Event_Types` 和 :ref:`Instructor_Event_Types` 将提供数据包中每种类型事件的描述。要查找有关特定事件类型的信息，请参阅
:ref:`event_list`.
...
**详细信息：** 用户访问的网站，如 ``courses.edx.org``。
...
**历史：** 包括从2014年5月7日加入的包括
``name`` 字段的服务器和移动的事件。当这个字段出现一个事件，它便取代
``event_type`` 字段。
...
**详细内容：** 当事件被触发时，用户访问该 '$URL' 页面。

对于在移动设备上发起视频事件，通过视频组件来确定URL。
...
**详细信息：** 该 URI 来自 HTTP Referer 的request-header 字段. 想了解更多信息请参阅 `Referer`_的HTTP/1.1 头字段。
...
**详细信息：** 这32个字符的值是标识用户的会话密钥，所有的浏览器事件和服务器T :ref:`enrollment<enrollment>` 事件都会包括会话的值，其他的服务器事件和移动事件不包括会话的值。
...
.. 注意:: 有时候，一个事件会被空白的O ``username``
 值所记录。当用户注销，或登录会话超时而浏览器窗口保持打开状态时会发生这种情况。后续操作被记录，但是系统不能提供用户标识符。Edx建议您在分析过程中忽略这些事件。
...
本节列出通常由学生发起的事件，它们往往由于学习管理系统（LMS）交互产生的，而不是教师仪表板等。

.. contents::
  :local:
  :depth: 1

下面将说明每个事件表示什么，以及起源于哪些系统组件，事件随着时间的推移所做的任何更改历史，还有常见的上下文或事件字段包含的任何额外成员字段。有关常见 ``context`` 或 ``event`` 字段的详细信息，请参见 :ref:`common`。``event_source`` 字段 (见上下文的 :ref:`common`  部分)的值可以区分在服务器（一个请求的处理过程中）上发起的事件和浏览器上发起的事件（在JavaScript）。
...
``edx.course.enrollment.activated`` 和 ``edx.course.enrollment.deactivated``
*****************************************************************************

响应于一个学生完成课程注册后，服务器发出这些事件。

* 当学生在课程注册时，服务器会发出 ``edx.course.enrollment.activated`` 事件。例如，但一个学生在edx.org网站点击报名参加课程，服务器会发出此事件。

* 当学生在注销课程时，服务器会发出 ``edx.course.enrollment.deactivated`` 事件。例如，当学生在edx.org网站单击取消注册课程，服务器会发出这个事件。

此外，课程团队成员的活动也产生招生事件。对于课程团队成员活动产生的事件，见 :ref:`instructor_enrollment` .

**事件来源**：服务器

**历史**： 这些招生事件添加于2013.12.03；2014.03.07，增加了 ``name`` 字段；所有的招生事件均包括一个 ``name`` 字段和一个 ``event_type`` 字段。
...
.. list-table::
   ...
   * - ``course_id``
     - string
     - 学生注册或取消注册课程。如果有外部工具或取消注册的学生，此字段包含一个值并且 ``context.course_id`` 字段为空。

   * - ``mode``
     - string
     - 'audit', 'honor', 'professional', 'verified'. 标识学生的注册模式。
   * - ``user_id``
     - number
     - 标识注册或取消注册的学生
...
当学生的 ``student_courseenrollment.mode`` 转向不同模式的改变过程完成后，服务器会发出 ``edx.course.enrollment.mode_changed`` 事件。
...
.. list-table::
   ...
   * - ``mode``
     - string
     - 'audit', 'honor', 'professional', verified'. 标识学生新的注册模式
   * - ``user_id``
     - number
     - 标识注册模式改变的学生
...
在有验证（即vefified）证书选项的课程中注册了 'audit' 或者 'honor'  ``student_courseenrollment.mode`` 的学生会在他们的仪表盘上看到关于该课程**Challenge Yourself**的链接。当学生点击这个选项，浏览器发出该事件，为了学生去“验证”的 ``student_courseenrollment.mode`` 将开始更新。请参看 :ref:`student_courseenrollment` 。
...
除了常见的 :ref:`common<context>`  ``context`` 成员字段, 这一事件类型还包括以下的 ``context`` 成员字段。
...
.. list-table::
   ...
   * - ``mode``
     - string
     - 当使用者点击 **Challenge Yourself**中 'audit' 或者
       'honor'.中的注册模式
...
``page_close`` 事件起源于JavaScript Logger自身。

**组件**: JavaScript Logger
...
* 当用户在一个序列的单元之间跳跃 ``seq_goto`` 被发出。
* 当用户导航到下一序列中的单元时 ``seq_next`` 被发出。
* 当用户导航到之前序列的单元中 ``seq_prev`` 被发出。
...
.. list-table::
   ...
   * - ``new``
     - number
     - 对于 ``seq_goto`` , 为跳转到的单元索引

       对于 ``seq_next`` 和 ``seq_prev`` , 为正在导航的单元索引

   * - ``old``
     - number
     - 对于 ``seq_goto`` , 未跳转前的单元索引

       对于 ``seq_next`` 和 ``seq_prev`` , 为导航离开的单元索引
